...
def read_dataset_partition_dict(dataset_name, seed):
    return pkl.load(open(f"data/dataset_partitions/{dataset_name}_seed{seed}.pkl", 'rb'))
...
def get_file_name(args):
    if config.Constants.file_name_suffix is not None:
        ...
        operator = args.pair_aggregation
        file_name_suffix = (f"{file_name_suffix}_Operator={operator}")
    else:
...
def generate_final_result_csv(results_dict, args):
    ...
    #     generate_final_results_blocking(results_dict, results_path, file_name)
    else:
